...
import subprocess
import logging
from config import VIDEO_DIR,VIDEO_BITRATE, CAMERA_WIDTH, CAMERA_HEIGHT, CAMERA_FRAMERATE, MIN_FREE_DISK_MB
...
def build_record_command(binary, output_path):
    cam_cmd = [binary, "--width", str(CAMERA_WIDTH), "--height", str(CAMERA_HEIGHT),
               "--framerate", str(CAMERA_FRAMERATE), "bitrate", str(VIDEO_BITRATE), "--codec", "h264",
               "-t", "0", "-o", "-", "--nopreview"]
    ffmpeg_cmd = ["ffmpeg", "-y", "-fflags", "+genpts","-r", str(CAMERA_FRAMERATE), "-i", "pipe:0",
